...
package cc.minos.codec.mov.boxs {
    ...
    public class StblBox extends Box {
        ...
        override protected function init():void
        {
            ...
            //stts
            _sttsBox = getBox(Mov.BOX_TYPE_STTS).shift() as SttsBox;

            //stsz
            var sizes:Vector.<uint>;
            try{
                var stszBox:StszBox = getBox(Mov.BOX_TYPE_STSZ).shift() as StszBox;
                sizes = stszBox.sizes;
            }catch(er:Error)
            {
                trace(er.message);
            ...
            var stscBox:StscBox = getBox(Mov.BOX_TYPE_STSC).shift() as StscBox;
            ...
            try
            {
                var stcoBox:StcoBox = getBox(Mov.BOX_TYPE_STCO).shift() as StcoBox;
            ...
            try
            {
                _stssBox = getBox(Mov.BOX_TYPE_STSS).shift() as StssBox;
            ...
            _stsdBox = getBox(Mov.BOX_TYPE_STSD).shift() as StsdBox;
        }
